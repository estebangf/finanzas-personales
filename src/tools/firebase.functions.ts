...
export const createMovement = async (movement: MovementModel, firestore: Firestore) => await new Promise<void>((resolve, reject) => {
...
export const getMovement = async (movement_id: string, firestore: Firestore) => await new Promise<MovementModel>((resolve, reject) => {
  const queryMovements = doc(firestore, MOVEMENTS_COLLECTION, movement_id).withConverter(movementConverter)
  getDoc(queryMovements).then(result => {
    if (result.exists()) { resolve(result.data()) } else { reject('Movimiento inexistente') }
  })
})

export const updateMovement = async (movement: MovementModel, firestore: Firestore) => await new Promise<void>((resolve, reject) => {
...
export const createWallet = async (wallet: WalletModel, firestore: Firestore) => await new Promise<void>((resolve, reject) => {
...
export const updateWallet = async (wallet: WalletModel, firestore: Firestore) => await new Promise<void>((resolve, reject) => {
...
export const updateWalletBalance = async (wallet_id: string, mod: number, firestore: Firestore) => await new Promise<void>((resolve, reject) => {
  const queryWallets = doc(firestore, WALLETS_COLLECTION, wallet_id).withConverter(walletConverter)
